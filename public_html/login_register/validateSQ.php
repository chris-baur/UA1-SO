...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($validData){
        // verify answer_one
        if(validateString('answer_one') && strlen($_POST['answer_one']) >= 6){
            $log->lwrite('answer_one passed prelim validaton');
            $ans1 = htmlentities($_POST['answer_one']);
            if (answer1_verify($a1, $account -> getAnswerOne())){
                $log->lwrite('answer_one is valid');
            ...
            //answer_one doesnt match
            else{
                //increaseAttemptCounter($userObj);
                $log->lwrite('answer_one incorrect.');
                $invalidArray['answer_one'] = 'Incorrect answer_one entered';
                showUserError();
            }
        }
        //incorrect syntax/length entered
        else{
            //increaseAttemptCounter($userObj);
            $log->lwrite('answer_one failed prelim verification');
            $invalidArray['answer_one'] = 'Incorrect answer_one entered. Check length/syntax';
            showUserError();
        }
    }

    if(validateString('answer_two') && strlen($_POST['answer_two']) >= 6){
            $log->lwrite('answer_two passed prelim validaton');
            $ans1 = htmlentities($_POST['answer_two']);
            if (answer1_verify($a2, $account -> getAnswertwo())){
                $log->lwrite('answer_two is valid');
                $_SESSION['userid'] = $account -> getId();
                $_SESSION['username'] = $account -> getUsername();
                session_regenerate_id();
                // redirect to user home page
            ...
            //answer_two doesnt match
            else{
                //increaseAttemptCounter($userObj);
                $log->lwrite('answer_two incorrect.');
                $invalidArray['answer_two'] = 'Incorrect answer_two entered';
                showUserError();
            }
        }
        //incorrect syntax/length entered
            ...
            //increaseAttemptCounter($userObj);
            $log->lwrite('answer_two failed prelim verification');
            $invalidArray['answer_two'] = 'Incorrect answer_two entered. Check length/syntax';
            showUserError();
        }
